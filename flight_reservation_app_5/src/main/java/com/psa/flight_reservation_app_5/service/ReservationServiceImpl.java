...
//		System.out.println(request.getLastName());
		...
		reservation.setFlight(flight);//automatically flight id will save, when we give flight object
		reservation.setPassenger(passenger);//automatically passenger id will save, when we give passenger object
		reservation.setCheckedIn(false);
		reservation.setNumberOfBags(0);
		
		reservationRepo.save(reservation);
		
		String filePath = "C:\\Users\\JOHN\\Documents\\workspace-spring-tool-suite-4-4.8.0.RELEASE\\flight_reservation_app_5\\tickets\\reservation"+reservation.getId()+".pdf";
		pdfGenerator.generateItinerary(reservation, filePath);
		emailUtil.sendItinerary(passenger.getEmail(), filePath);
		
		return reservation;// service layer not interact with view so passing reservation object to the controller controller interact with view
	}
